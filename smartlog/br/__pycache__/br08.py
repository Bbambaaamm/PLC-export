...
def read_br08(data, last_data, pending_metrics, executed_timestamp=None) -> None:
    ...
    if existing is not None:
        ...
        # ✅ ignoruj reset 0/0, pokud už máme uložený validní výsledek
        # a zaloguj to jen jednou pro daný box_id
        ...
        # jakmile přijde jiný validní stav, reset flag smažeme
        ...
        # přidej do pending jen pokud tam není stejná kombinace (box+kod+směr)
        if not any(
            e.get("box_id") == box_id
    ...
    # -----------------------------------------------------------------
    # ✅ NOVÝ PRŮJEZD
    # nový záznam zakládej jen tehdy, když data nejsou reset 0/0
    # -----------------------------------------------------------------
    if is_reset_state:
        return

    # nový validní průjezd => reset flag smažeme
    last_data["br08_ignored_reset_box_id"] = None
    ...
    # přidej do pending, pokud tam není stejná kombinace
    if not any(
    ...
    # -----------------------------------------------------------------
    # ✅ PREFIX COUNTER
    # Počítá nové validní průjezdy podle prefixu box_id
    # např. 10..., 05..., 15..., 20...
    # -----------------------------------------------------------------
    prefix = box_id[:2]
